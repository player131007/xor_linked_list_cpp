...
public:
	...
	constexpr xor_list_iterator() : prev(nullptr), curr(nullptr) {}
	constexpr xor_list_iterator(node* prev,node *curr) : prev(prev),curr(curr) {}
	constexpr xor_list_iterator(const xor_list_iterator &other) : prev(other.prev),curr(other.curr) {}

	constexpr operator bool() const noexcept { return curr != nullptr; }
	constexpr friend bool operator==(const xor_list_iterator &a, const xor_list_iterator &b) noexcept {
	...
	constexpr xor_list_iterator operator++(int) {
		auto res = *this;
		++*this;
		return res;
	}
